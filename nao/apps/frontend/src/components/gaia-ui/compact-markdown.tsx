...
// (registry item "tool-calls-section", file "registry/new-york/ui/compact-markdown.tsx")
// Vendored verbatim into nao. No modifications.
...
export function CompactMarkdown({ content, className = '' }: CompactMarkdownProps) {
	...
	const baseClasses =
		'bg-zinc-900/50 rounded-xl p-3 max-h-60 overflow-y-auto text-xs text-zinc-400 w-fit max-w-[32rem]';

	// For structured data, render as preformatted text
	if (isStructured) {
		const displayText =
			typeof data === 'string' ? formatJsonLikeString(data) : JSON.stringify(data, null, 2);
	...
	return <div className={`${baseClasses} leading-relaxed ${className}`}>{textContent}</div>;
}
